...
<head>
    <meta charset="UTF-8">
    <title>Ubicaciones Físicas</title>
...
<body class="bg-base-100 ">
    ...
    <div class="drawer drawer-bottom bottom-0">
        ...
        <div class="drawer-content flex flex-col">
            ...
            <div class="bg-base-300 p-5 rounded-lg content-top m-5">
                ...
                <h2 class="text-lg font-bold">Ubicaciones Físicas</h2>
                <p>En este panel puedes agregar, modificar y eliminar las ubicaciones físicas.</p>

                <form action="/docudigitalsds/PhysicalLocationsController" method="post">
                    <div class="join w-full flex mt-5">
                        <div class="flex-grow">
                            <div>
                                <input name="nuevaUbicacionFisica" class="input input-bordered join-item w-full" placeholder="Ingresa el nombre de la nueva ubicación física" required/>
                        ...
                        <tbody>
                            ...
                            %>
                                    ...
                                    <tr class="bg-base-100">
                                        <!-- Muestra cada nombre de ubicación física dentro de la columna correspondiente -->
                                        <td><%= location.getIdUbicacionFisica() %></td>
                                        <td><%= location.getNombre() %></td>
                                        <td class="pr-0">
                                            <div class="flex justify-end gap-4">

                                                <!-- Botón de editar con modal -->
                                                <button class="btn btn-sm text-primary" onclick="openModal('<%= location.getNombre() %>', '<%= location.getIdUbicacionFisica() %>')">Editar</button>

                                                <!-- Modal -->
                                                <dialog id="editModal" class="modal">
                                                    <div class="modal-box pb-7">
                                                        <h3 class="font-bold text-lg pb-5">Editar Ubicación Física</h3>

                                                        <form action="/docudigitalsds/PhysicalLocationsController" method="post">

                                                            <input type="hidden" name="action" value="update" />

                                                            <input type="hidden" name="idUbicacionFisica" id="locationId" />

                                                            <input type="text" name="nombreUbicacionFisica" id="editLocationNameInput" placeholder="Nuevo nombre" class="input input-bordered w-full max-w " />
                                                ...
                                                <form action="/docudigitalsds/PhysicalLocationsController" method="post">
                                                    ...
                                                    <input type="hidden" name="idUbicacionFisica" value="<%= location.getIdUbicacionFisica() %>" />
    ...
    <script>
        ...
        // Script para abrir el modal con el nombre de la ubicación física
        function openModal(locationName, locationId) { 
            var modal = document.getElementById('editModal');
            document.getElementById('locationId').value = locationId; 
            document.getElementById('editLocationNameInput').value = locationName;
            modal.showModal();
        }
    </script>
